...
from spydiff import find_nw_beam, clean_difmap, CLEAN_difmap, find_bbox, rebase_CLEAN_model
...
if do_clean:
    ...
    if not os.path.exists(os.path.join(data_dir, f"{base_name}_x.fits")):
        ...
        # Rebase
        rebase_CLEAN_model(uvfits_x, os.path.join(data_dir, f"rebased_{source}_y.uvf"), "i", common_mapsize, nw_beam_x,
                           source_ccfits=os.path.join(data_dir, f"{source}_deep_{deep_factor}_wins_y.fits"),
                           source_difmap_model=None, noise_scale_factor=0.01, need_downscale_uv=False, remove_cc=True)
        rebase_CLEAN_model(uvfits_x, os.path.join(data_dir, f"rebased_{source}_j.uvf"), "i", common_mapsize, nw_beam_x,
                           source_ccfits=os.path.join(data_dir, f"{source}_deep_{deep_factor}_wins_j.fits"),
                           source_difmap_model=None, noise_scale_factor=0.01, need_downscale_uv=False, remove_cc=True)
        rebase_CLEAN_model(uvfits_x, os.path.join(data_dir, f"rebased_{source}_u.uvf"), "i", common_mapsize, nw_beam_x,
                           source_ccfits=os.path.join(data_dir, f"{source}_deep_{deep_factor}_wins_u.fits"),
                           source_difmap_model=None, noise_scale_factor=0.01, need_downscale_uv=False, remove_cc=True)
    ...
    #                  noise_to_use="V", shift=shift_x)
        CLEAN_difmap(f"rebased_{source}_u.uvf", "i", common_mapsize, f"{base_name}_u.fits", restore_beam=nw_beam_x,
                     boxfile=wins, working_dir=data_dir, uvrange=common_uvrange,
                     box_clean_nw_niter=500, clean_gain=0.03, dynam_su=20, dynam_u=8, deep_factor=deep_factor,
                     remove_difmap_logs=True, save_noresid=None, save_resid_only=None, save_dfm=None,
                     noise_to_use="V")
        CLEAN_difmap(f"rebased_{source}_y.uvf", "i", common_mapsize, f"{base_name}_y.fits", restore_beam=nw_beam_x,
                     boxfile=wins, working_dir=data_dir, uvrange=common_uvrange,
                     box_clean_nw_niter=500, clean_gain=0.03, dynam_su=20, dynam_u=8, deep_factor=deep_factor,
                     remove_difmap_logs=True, save_noresid=None, save_resid_only=None, save_dfm=None,
                     noise_to_use="V", shift=shift_y)
        CLEAN_difmap(f"rebased_{source}_j.uvf", "i", common_mapsize, f"{base_name}_j.fits", restore_beam=nw_beam_x,
                     boxfile=wins, working_dir=data_dir, uvrange=common_uvrange,
                     box_clean_nw_niter=500, clean_gain=0.03, dynam_su=20, dynam_u=8, deep_factor=deep_factor,
                     remove_difmap_logs=True, save_noresid=None, save_resid_only=None, save_dfm=None,
...
# FIXME: For M87 it is 0.2
ccimage_x = create_image_from_fits_file(os.path.join(data_dir, f"{source}_deep_0.1_wins_x.fits"))
