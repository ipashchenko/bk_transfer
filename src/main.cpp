...
//	std::vector<std::string> nu_observed_band{"u"};
    ...
    for(int i = 0; i < num_flares; i++){
        ...
        flaring_nfield = new FlareBKNField(&static_nfield, frac_amp, t_start_days, flare_width_pc, vfield, &geometry, nullptr);
		// Using adiabatically declined N-field as flare background
        // flaring_nfield = new FlareBKNField(&flare_base_nfield, frac_amp, t_start_days, flare_width_pc, vfield, &geometry, nullptr);
