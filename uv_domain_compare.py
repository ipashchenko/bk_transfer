...
save_dir = "/home/ilya/data/alpha/blind_clean/BK145/uv_clipping/fix_beam_bpa/radplot/rot90"
...
# sc = axes.scatter(u[toogood]/10**6, v[toogood]/10**6, c=1000*diff[toogood], cmap="plasma")
# sc = axes.scatter(-u[toogood]/10**6, -v[toogood]/10**6, c=1000*diff[toogood], cmap="plasma")
divider = make_axes_locatable(axes)
cax = divider.append_axes("right", size="3%", pad=0.00)
cb = fig.colorbar(sc, cax=cax)
cb.set_label(r"${\rmFrac. error}$")
# cb.set_label(r"${\rm Error (mJy)}$")
...
axes.invert_xaxis()
plt.savefig(os.path.join(save_dir, "fracerror_8GHz_2ridges_rot90.png"), bbox_inches="tight", dpi=600)
plt.show()
